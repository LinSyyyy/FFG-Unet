...
from lib.utils.module import Attention, PreNorm, FeedForward, CrossAttention
...
class Discriminator(nn.Module):
    def __init__(self, input_dim):
        ...
        self.fc1 = nn.Linear(input_dim, 256)  # 第一个全连接层
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(self.fc3(x))  # 输出概率
        return x
class Swin_Unet(nn.Module):
    def __init__(self, dim, n_class=3, in_ch=3):
        super().__init__()
        ...
        self.layer44 = Decoder(dim, dim, dim // 2)
        self.layer55 = Decoder(dim // 2, dim // 2, dim // 4)
    ...
    def forward(self, x1):
        ...
        out1 = F.relu(F.interpolate(self.dbn3(d3), scale_factor=(2, 2), mode='bilinear'))
        out = self.upp(out1)

        return out, e1, e2, e3, e4,d1,d2,d3,out1

class Swin_UnetA(nn.Module):
    def __init__(self, dim, n_class=3, in_ch=3):
        super().__init__()
        self.encoder = SwinTransformer(depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], drop_path_rate=0.5,
                                       embed_dim=128)
        # self.encoder.init_weights(pretrained=None)
        self.encoder.init_weights(
            pretrained="C:/Users/msi/Desktop/DS-TransUNet-master/checkpoints/swinv2_tiny_patch4_window8_256.pth")
...
#         out = self.encoder(x1)
#         e1, e2, e3, e4 = out[0], out[1], out[2], out[3]
#         # fenlei = torch.flatten(e4, 1)
#         # fenlei = self.final(fenlei)
#         # fenlei = torch.sigmoid(fenlei)
#         d1 = self.layer1(e4, e3)
#         d2 = self.layer2(d1, e2)
#         d3 = self.layer3(d2, e1)
#         out1 = F.relu(F.interpolate(self.dbn3(d3), scale_factor=(2, 2), mode='bilinear'))
...
from lib.model_B import Swin_UnetB
# from Unext_ST import Swin_UnetB
class double_Swin_Unet2(nn.Module):
    def __init__(self, dim=128, n_class=3, in_ch=3):
        super(double_Swin_Unet2, self).__init__()
        self.model_A = Swin_UnetA(dim, n_class,in_ch)

        # for param in self.model_A.parameters():
        #     param.requires_grad = False
        self.model_B = Swin_UnetB(dim, n_class,in_ch)
        self.mse_loss = nn.MSELoss()
        self.reshape_size = 256
        self.discriminator = Discriminator(input_dim=65536).cuda()
        self.criterion = BCEDiceLoss()
        self.drop = nn.Dropout(0.1)

    def forward(self,x1,x2,test=False):
        s_al = None
        out1, e1, e2, e3, e4 = self.model_A(x1)
        out2, e11, e22, e33, e44 ,d11, d22, d33 = self.model_B(x2)
        #------判别器------------------
        # e4 = self.drop(e4)
        # e44 = self.drop(e44)
        # combined_features = torch.cat((e4.view(e4.size(0), -1), e44.view(e44.size(0), -1)), dim=0)
        # labels_real = torch.ones(e4.size(0), 1).cuda()  # 正常样本标签,1
        # labels_fake = torch.zeros(e44.size(0), 1).cuda()  # 异常样本标签,0
        # combined_labels = torch.cat((labels_real, labels_fake), dim=0) #11111,00000(先正常后异常)
        # discriminator_output = self.discriminator(combined_features)
        # loss4 = F.binary_cross_entropy(discriminator_output, combined_labels)

        # 计算生成器的对抗损失
        generator_labels = torch.ones(e44.size(0), 1).cuda()  # 生成样本标签, 1
        discriminator_output_gen = self.discriminator(e44.view(e44.size(0), -1))  # 生成的特征
        loss4 = F.binary_cross_entropy(discriminator_output_gen, generator_labels)

        # ------和原图损失------------------------------------
        loss5 = self.mse_loss(out2, x1)
        # loss5 = self.cos_similarity(out2, x1)
        # ------异常图像-------------------------------------
        loss1 = self.cos_similarity(e1, d33)
        loss2 = self.cos_similarity(e2, d22)
        loss3 = self.cos_similarity(e3, d11)

        loss = loss1 + loss2 + loss3 +loss4+loss5
        if test:
            ano_map1, loss1, mse1 = get_ano_map(e1, d33)
            ano_map2, loss2, mse2 = get_ano_map(e2, d22)
            ano_map3, loss3, mse3 = get_ano_map(e3, d11)
            ano_map4 = abs(out2-x2)

            ano_map1 = nn.functional.interpolate(ano_map1, size=(256, 256), mode='bilinear',
                                                 align_corners=True)
            ano_map2 = nn.functional.interpolate(ano_map2, size=(256, 256), mode='bilinear',
                                                 align_corners=True)
            ano_map3 = nn.functional.interpolate(ano_map3, size=(256, 256), mode='bilinear',
                                                 align_corners=True)
            ano_map4 = nn.functional.interpolate(ano_map4, size=(256, 256), mode='bilinear',
                                                 align_corners=True)

            ano_map4 = torch.mean(ano_map4, dim=1, keepdim=True)


            # s_al = (ano_map1 + ano_map2 + ano_map3+ ano_map4).squeeze().cpu().numpy()
            s_al = (ano_map1 + ano_map2 + ano_map3).squeeze().cpu().numpy()

            s_al = gaussian_filter(s_al, sigma=4)

        return loss, s_al,out1,out2

if __name__ == '__main__':
    print('#### Test Case ###')
    # x = Variable(torch.rand(2, 3,64,64)).cuda()
    inx = torch.randn(1, 3, 256, 256).cuda()
    inx2 = torch.randn(1, 3, 256, 256).cuda()
    model = double_Swin_Unet2(128, 3,3).cuda()
    # model = Swin_Unet(128,3,3).cuda()
    loss, s_al, out1, out2 = model(inx,inx2)
    print(loss)
