...
import einops
from lib.network_swinir import UpsampleOneStep
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # ----------------linear------------------
        self.w_g = nn.Parameter(torch.randn(dim * 2, 1))
        self.to_query = nn.Linear(dim, dim * 2)
        self.to_key = nn.Linear(dim, dim * 2)

        self.Proj2 = nn.Linear(dim * 2, dim * 2)
        self.final = nn.Linear(dim * 2, dim)

    def forward(self, x,De=False, mask=None):
        """ Forward function.
        Args:
        ...
        if De:
            ...
            # ---------------------------线性注意力机制,的Encoder里用----------------------------------------------
            B_, N, C = x.shape
            winDowsize = self.window_size[0] * self.window_size[1]
            q = self.to_query(x)
            k = self.to_key(x)
            q = torch.nn.functional.normalize(q, dim=-1)  # BxNxD
            k = torch.nn.functional.normalize(k, dim=-1)  # BxNxD
            q_weight = q @ self.w_g  # BxNx1 (BxNxD @ Dx1)
            A = q_weight * self.scale  # BxNx1
            A = torch.nn.functional.normalize(A, dim=1)  # BxNx1
            G = torch.sum(A * q, dim=1)  # BxD
            G = einops.repeat(
                G, "b d -> b repeat d", repeat=k.shape[1]
            )  # BxNxD
            attn = self.Proj2(G * k) + q  # BxNxD
            attn = attn.reshape(B_, -1, winDowsize, winDowsize)
            B1, M, D1, D2 = attn.shape
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix,De=False):
        """ Forward function.
        Args:
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=attn_mask,De = De)  # nW*B, window_size*window_size, C
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W,De = False):
        """ Forward function.
        Args:
            x: Input feature, tensor size (B, H*W, C).
            H, W: Spatial resolution of the input feature.
...
class SwinDecoder(nn.Module):
    ...
    def forward(self, x,De=False):
        ...
        x_out, H, W, x, Wh, Ww = self.layer(x, H * 2, W * 2,De=De)
...
class Swin_Decoder(nn.Module):
    def __init__(self, in_channels, depths, num_heads):
        ...
        self.up = SwinDecoder(in_channels, depths=depths, num_heads=num_heads,window_size=8)
...
class Swin_UnetB(nn.Module):
    def __init__(self, dim, n_class=3, in_ch=3):
        ...
        self.final = nn.Conv2d(128, n_class, kernel_size=1)
        self.down1 = nn.Conv2d(in_ch, dim // 4, kernel_size=1, stride=1, padding=0)
        self.down2 = conv_block(dim // 4, dim // 2)
        embed_dim = 128
        # self.conv_after_body = nn.Sequential(nn.Conv2d(embed_dim, embed_dim * 2, 3, 1, 1),
        ...
        self.conv_after_body = nn.Sequential(
            ...
            nn.Conv2d(embed_dim * 2, embed_dim * 4, 1, 1, 0),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(embed_dim * 4, embed_dim * 8, 3, 2, 1),  # 修改步长为2
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(embed_dim * 8, embed_dim * 16, 3, 2, 1),  # 修改步长为2
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
        ...
        self.upsample = UpsampleOneStep(2, embed_dim, 3,
    ...
    def forward(self, x1):
        ...
        return out, e1, e2, e3, e4,d1,d2,d3
...
if __name__ == '__main__':
    ...
    inx = torch.randn(5, 3, 256, 256).cuda()
    inx2 = torch.randn(5, 3, 256, 256).cuda()
    model = Swin_UnetB(128, 3,3).cuda()
    # model = Swin_Unet(128,3,3).cuda()
    out, e1, e2, e3, e4,d1,d2,d3 = model(inx)
    print(out.shape)
